...
public class DaoBeanDefinitionRegistryPostProcessor implements BeanDefinitionRegistryPostProcessor {
    ...
    <bean id="customerDao" parent="abstractDao" >
        <property name="proxyInterfaces" value="com.bleum.canton.sample.dao.CustomerDao"/>
        <property name="target">
        	<bean parent="abstractDaoTarget">
        		<property name="type" value="com.bleum.canton.sample.entity.SampleCustomer"/>
            </bean>
        </property>
    </bean>
	 */
	public void postProcessBeanDefinitionRegistry(
		...
		for (BeanDefinition candidate : candidates) {
			...
			if(entityType == null || entityType.equals("")){
				try {
					daoClass = Class.forName(candidate.getBeanClassName());
				} catch (ClassNotFoundException e) {
					debug("can not find class, exception:"+e);
					continue;
				}
			...
			BeanDefinition daoBean = buildBeanDefinition(candidate.getBeanClassName(), entityType);

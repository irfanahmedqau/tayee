/**
 * 
 */
package com.bleum.canton.checkout.service;

import static org.junit.Assert.*;

import org.junit.After;
import org.junit.Before;
import org.junit.Test;

/**
 * @author walter.chen
 *
 */
public class CheckOutProcessorTest {

	/**
	 * @throws java.lang.Exception
	 */
	@Before
	public void setUp() throws Exception {
	}

	/**
	 * @throws java.lang.Exception
	 */
	@After
	public void tearDown() throws Exception {
	}

	@Test
	public void test() {
		fail("Not yet implemented");
	}
	@Test
	public void testInitialCheckOut()throws Exception{
		
	}
	@Test
	public void testPostToOms()throws Exception{
		
	}
	@Test
	public void testSaveAccountInfo()throws Exception{
		
	}
	@Test
	public void testSavePaymentAndShippingMethod()throws Exception{
		
	}
	@Test
	public void testSaveInvoiceInfo()throws Exception{
		
	}
}
